...
func createRecentItems(chatRoomId: String, users: [User]) {
    ...
    FirebaseReference(.Recent).whereField(kCHATROOMID, isEqualTo: chatRoomId).getDocuments { (snapshot, error) in
        ...
        for userId in memberIdsToCreateRecent {
            ...
            let recentObject = RecentChats(id: UUID().uuidString, chatRoomId: chatRoomId, senderId: senderUser.id, senderName: senderUser.lastName, recieverId: receiverUser.id, recieverName: receiverUser.lastName, date: Date(), memberIds: [senderUser.id, receiverUser.id], lastMessage: "", unreadCounter: 0, avatarLink: receiverUser.avatarLink)
...
func chatRoomIdFrom(user1Id: String, user2Id: String) -> String {
    ...
    //chatroomID is created by adding together both users Ids together
