...
class BlockedUsersViewController: UIViewController, UITableViewDataSource, UITableViewDelegate, ContactsTableViewCellDelegate {
    ...
    func tableView(_ tableView: UITableView, commit editingStyle: UITableViewCell.EditingStyle, forRowAt indexPath: IndexPath) {
        ...
        var tempBlockedUsers = FUser.currentUserFunc()!.blockedUsers
    ...
    func loadUsers() {
        ...
        ///access current user
        if FUser.currentUserFunc()!.blockedUsers.count > 0 {
            
            hud.show(in: self.view)
            
            getUsersFromFirestore(withIds:  FUser.currentUserFunc()!.blockedUsers) { (allBlockedUsers) in
